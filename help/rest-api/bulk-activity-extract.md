...
| fields | Array[String] | No | Optional array of strings containing field values. The listed fields are included in the exported file. By default, the following fields are returned: <ul><li>`marketoGUID`</li><li>`leadId`</li><li> `activityDate` </li><li>`activityTypeId` </li><li>`campaignId`</li><li> `primaryAttributeValueId` </li><li>`primaryAttributeValue`</li><li> `attributes`</li></ul>. This parameter can be used to reduce the number of fields that are returned by specifying a subset from the list above:`"fields": ["leadId", "activityDate", "activityTypeId"]`. An additional field `actionResult` can be specified to include the activity action: `("succeeded", "skipped", or "failed")`. |
