...
description: Marketo Asset REST API for Fragments, covering query by Id and browse with status, getting content, creating and updating HTML, Text, and dynamic content.
...
Fragments are reusable HTML components which can be embedded into Emails and Landing Pages and which can be segmented for dynamic content. Fragments don't have associated templates, and can be created and deployed within other assets within Marketo.

## Query

Querying Fragments follows the standard pattern for assets, except it does not have a By Name method. Both the [By Id](https://developer.adobe.com/marketo-apis/api/asset/#tag/Fragments/operation/getSnippetByIdUsingGET) and [Browse](https://developer.adobe.com/marketo-apis/api/asset/#tag/Fragments/operation/getSnippetUsingGET) methods allow the use of the status field to retrieve either approved or draft versions of the snippet.
...
```
GET /rest/asset/v1/Fragments.json?maxReturn=3
```
...
Fragments follow the complex asset creation pattern, where the call to [create snippet](https://developer.adobe.com/marketo-apis/api/asset/#tag/Fragments/operation/createSnippetUsingPOST), and its content are made separately, so the first call must be to the create endpoint, with an optional description.   Data is passed as x-www-form-urlencoded, not as JSON.

```
POST /rest/asset/v1/Fragments.json
```
...
[Updating metadata](https://developer.adobe.com/marketo-apis/api/asset/#tag/Fragments/operation/updateSnippetUsingPOST) is also done by id. Only name and description can be updated:
...
Fragments have endpoints available for approving, unapproving, and discarding drafts, which follow the standard asset pattern. A snippet must be in draft status for it to be approved.
...
The `unapprove` endpoint can only be used on approved Fragments.
